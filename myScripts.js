...
$("document").ready(function () {
  ...
  $("header").append('<div class="bigTitle col-6"><h1>ThankYou JADI</h1><p>for all this awesome contents that you spend time to make for us... your the best</p></div>');

  // $("#section1 .section-title").append("<h4>Video Toturials<hr></h4>");
  for (let i = 1; i < img.length; i++) {
    tmpHtml = '<div class="gallery-card col-lg-4 col-md-6 col-12">' + '<a href="' + img[i].link
      + '" target="__blank">' + '<img src="img/' + img[i].img + '">'
      + "<br>" + img[i].title + '</a>' + '</div>';
    $("#section1").parent().append(tmpHtml);
  }

  // $("#section2 .section-title").append("<h4>Books<hr></h4>");
  for (let i = 1; i < book.length; i++) {
    tmpHtml = '<div class="gallery-card col-lg-4 col-md-6 col-12">' + '<a href="' + book[i].link
      + '" target="__blank">' + '<img src="img/' + book[i].img + '">'
      + "<br>" + book[i].title + '</a>' + '</div>';
    $("#section2").parent().append(tmpHtml);
  }

  // $("#section3 .section-title").append("<h4>Podcast<hr></h4>");
  for (let i = 1; i < podcast.length; i++) {
    tmpHtml = '<div class="gallery-card col-lg-4 col-md-6 col-12">' + '<a href="' + podcast[i].link
      + '" target="__blank">' + '<img src="img/' + podcast[i].img + '">'
      + "<br>" + podcast[i].title + '</a>' + '</div>';
